...
class decimator_ccf(gr.hier_block2):
	...
	gr.hier_block2.__init__(self, "gsm_decimator_ccf",
				gr.io_signature(1, 1, gr.sizeof_gr_complex),
				gr.io_signature(decim, decim, gr.sizeof_gr_complex))
        ...
        if (taps is not None) and (len(taps) > 0):
            ...
            while not made:
                try:
                    self._taps = firdes.low_pass(1, self._decim, bw, bw+tb, ripple, atten)
                    made = True
                except RuntimeError:
                    ripple += 0.01
                    made = False
        ...
        self.pfb = grgsm.gsm_decimator_ccf(self._decim, self._taps, self._channel,
